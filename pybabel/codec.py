...
# Last modified: Fri Nov  9 12:59:10 2018 mstenber
# Edit time:     97 min
#
"""
...
class Packet(CStruct):
    ...
    def decode_buffer(self, x):
        ...
        if self.magic != Packet.magic:
            _decode_error("wrong magic", self)
        if self.version != Packet.version:
            _decode_error("wrong version", self)
        self.tlvs = list(decode_tlvs(
            x[self.format_size():self.format_size()+self.length]))
...
class Update(BodyTLV):
    ...
    keys = TLV.keys[:] + ['ae', 'flags', 'plen',
                          'omitted', 'interval', 'seqno', 'metric']
...
_tlvlist = [Pad1, PadN, AckReq, Ack, Hello,
            IHU, RID, NH, Update, RouteReq, SeqnoReq]
...
def ll_to_tlv_args(ip):
    b = ip.packed[8:]
    if not b:
        _decode_error("non-IPV4 address in ll_to_tlv_args", ip)
    return {'ae': 3, 'body': b}
...
def tlv_to_prefix(tlv):
    if tlv.ae not in _p_ae2len:
        _decode_error("unsupported af in tlv_to_prefix", tlv)
    el = _p_ae2len[tlv.ae]  # expected length
    if tlv.plen:
        sl = (tlv.plen + 7)//8
        if len(tlv.body) < sl:
            _decode_error("too short prefix", tlv)
        b = tlv.body[:sl]
    else:
        b = b''
    b = b + bytes(el - len(b))
...
def tlv_to_ip_or_ll(tlv):
    if tlv.ae not in _ae2len:
        _decode_error("unsupported af in tlv_to_ip_or_ll", tlv)
